...
{	
	...
	//whenever we somehow send a post request from our flutter app,
	//with the data of $channelName then we will get it with the $request object
	//and also with the $appId and $appCertificate we will be able to get our generated
	//agora access token that will be passed to our app through the api response
	
    //access token method
	public function get_rtc_token(Request $request){
		//channel name variable accessing with the help of input()
		//By using $request->input("channel_name"), 
		//it's clear that we're accessing input data from the request object. 
		$channelName = $request->input("channel_name");
		$appId = "--appId-from-agora-here--";
		$appCertificate = "--app-certificate-from-agora-here--";
		//will get from agora 
		$uid = 0;
		//how long users can conversation with each other in call
		$expireTimeInSeconds = 7200;//like 120min*60sec
		
		//based on the three parameters value agora will generate an accessToken for our app and make calling
		...
		//if empty or any error come
		return ['code'=>-1,'data'=>'','message'=>'token error'];
	}
}



//agora will make accessToken for us, we will get our accessToken from the
//below class
//we get the below clss,
// from this link: https://www.dbestech.com/tutorials/agora-and-php-integration
class Service
{
    public $type;
    public $privileges;
